...
class Report extends AbstractController
{
    ...
    public function actionIndex()
    {
        ...
        if (!in_array($filters['state'], ['', 'pending', 'cleared', 'suspicious', 'confirmed'], true))
        {
            $filters['state'] = '';
        }
        ...
        if ($filters['state'] !== '')
        {
            $where[] = 'a.review_state = ?';
            $params[] = $filters['state'];
        }
        if ($filters['risk_min'] > 0)
        {
            $where[] = 'a.risk_score >= ?';
            $params[] = $filters['risk_min'];
        }
        if ($filters['forum_id'] > 0)
        {
            $where[] = 'a.forum_id = ?';
            $params[] = $filters['forum_id'];
        }
        ...
        $rows = $db->fetchAll(
            'SELECT a.analysis_id, a.post_id, a.thread_id, a.user_id, a.forum_id,
             ...
             LEFT JOIN xf_thread t ON t.thread_id = a.thread_id' .
             $whereSql .
